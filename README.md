...
本次模板适配按 `2.2.1` 发布，绑定 `yss-spec-project-template@7373097`。`templateCommit` 会写入实例 metadata；“最新模板”指用户执行的 `npx create-yss-spec@latest` 所携带的最新已发布快照，CLI 运行时不会拉取模板仓库。实例会带上 YSS 前端技能叠加层（如 `ytable-usage`、`formily-foundation`）、`.cursorrules` 与 `.agents/rules/yss-ai-skills.md`；审查临时目录 `docs/.scratch/` 仍不进入快照。

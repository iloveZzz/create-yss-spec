...
本次模板适配按 `2.2.1` 发布，绑定 `yss-spec-project-template@2375746`。`templateCommit` 会写入实例 metadata；“最新模板”指用户执行的 `npx create-yss-spec@latest` 所携带的最新已发布快照，CLI 运行时不会拉取模板仓库。实例会带上已治理的 YSS 前端技能（如 `yss-page-module-development`、`ytable-usage`、`formily-foundation`）、`.cursorrules` 与 `.agents/rules/yss-ai-skills.md`；`high-fidelity-html-prototype` 等退役入口不再分发。

...
const snapshotExcludedInstancePaths = [
  ".gitignore",
  ".nvmrc",
  "wiki",
  "docs/adr",
...
test("interactive init generates a template instance in an empty directory", () => {
  ...
  for (const excludedInitPath of initExcludedInstancePaths) {
    assert.equal(
  ...
  assert.ok(fs.existsSync(path.join(targetDir, "docs/design/README.md")));
...
test("attach dry-run previews an arbitrary existing project without writing or deleting .git", () => {
  ...
  for (const excludedPath of [
    ...
    assert.doesNotMatch(
      result.stdout,
      new RegExp(`add: ${excludedPath.replaceAll(".", "\\.")}`),
    );
  }
...
test("attach applies management assets while preserving runtime files and .git", () => {
  ...
  for (const excludedPath of snapshotExcludedInstancePaths) {
    if (excludedPath === ".gitignore") {
      continue;
    }
    assert.equal(
      fs.existsSync(path.join(targetDir, excludedPath)),
      false,
      `${excludedPath} must not be added by attach`,
    );
  }
